...
class Product {
  ...
  getOne = async (id) => {
    ...
    if (fs.existsSync(`${this.path}productos.json`)) {
      const objects = await JSON.parse(
        fs.readFileSync(`${this.path}productos.json`)
      );
    ...
    if (fs.existsSync(`${this.path}productos.json`)) {
      ...
      if (query.name)
        objects.forEach((element) => {
          if (element.name == query.name) filtered.push(element);
        });
      if (query.category)
        objects.forEach((element) => {
          if (element.category == query.category) filtered.push(element);
        });
      if (query.price)
        objects.forEach((element) => {
          if (element.price == query.price) filtered.push(element);
        });
