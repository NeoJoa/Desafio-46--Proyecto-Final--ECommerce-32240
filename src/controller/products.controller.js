...
export default class ProductController {
  get = async (req, res, next) => {
    ...
    try {
      ...
      products.hasNextPage == true
        ? (nextLink = `http://localhost:8080/api/products/?limit=${limit}&page=${
            page + 1
          }&query=${query}`)
        : (nextLink = null);
      products.hasPrevPage == true
        ? (prevLink = `http://localhost:8080/api/products/?limit=${limit}&page=${
            page - 1
          }&query=${query}`)
        : (prevLink = null);
  ...
  put = async (req, res, next) => {
    ...
    try {
      ...
      if (req.user.user.role == "premium") {
        if (req.user.user.email != exists.owner)
          CustomError.createError({
            statusCode: 401,
            name: "Product isnt yours",
  ...
  delete = async (req, res, next) => {
    ...
    try {
      ...
      try {
        ...
        return res.send({
          status: "error",
          message: "El mail del dueño es inválido y no se ha podido avisar",
        });
      }
